...
import { Clock, Check, X, Plus, Calendar, LogIn, LogOut, Timer, AlertCircle } from 'lucide-react'
import { cn } from '@/lib/utils'

// ─── Types ────────────────────────────────────────────────────────────────────
interface Punch { type: 'in' | 'out'; time: Date; label: string }
...
function ClockWidget() {
  ...
  function handleClockIn() {
    ...
    setPunches(prev => [{ type: 'in', time: t, label: 'Clock In' }, ...prev])
    setJustActed('in'); setTimeout(() => setJustActed(null), 2000)
  }
  function handleClockOut() {
    const t = new Date()
    setClockedIn(false)
    setPunches(prev => [{ type: 'out', time: t, label: 'Clock Out' }, ...prev])
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {clockedIn && (
            <div className="text-right">
              <div className="flex items-center gap-1.5 text-white/70 text-xs mb-1 justify-end">
                <Timer className="w-3.5 h-3.5" /><span>Session time</span>
              </div>
        ...
        <div className="flex gap-3 mt-4">
          {!clockedIn ? (
            <button onClick={handleClockIn} className={cn('flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm transition-all', justActed === 'in' ? 'bg-emerald-400 text-white scale-95' : 'bg-white text-slate-900 hover:bg-emerald-50')}>
              ...
              {justActed === 'in' ? 'Clocked In ✓' : 'Clock In'}
            </button>
          ) : (
            <button onClick={handleClockOut} className={cn('flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm transition-all', justActed === 'out' ? 'bg-red-300 text-white scale-95' : 'bg-white/20 border border-white/30 text-white hover:bg-white/30')}>
              <LogOut className="w-4 h-4" />
              {justActed === 'out' ? 'Clocked Out ✓' : 'Clock Out'}
            </button>
          )}
            ...
            {[
              ...
              <div key={d.day} className="text-center">
                ...
                <div className={cn('h-12 rounded-lg flex items-end justify-center pb-1 relative overflow-hidden', d.status === 'active' ? 'bg-emerald-100' : d.status === 'today' ? 'bg-slate-50 border border-dashed border-slate-200' : 'bg-blue-50')}>
                  ...
                  <span className={cn('text-[10px] font-bold relative z-10', d.hours > 0 ? 'text-white' : 'text-slate-300')}>
...
export default function TimeOffPage() {
  ...
  return (
    <AppShell pageTitle="Time & Attendance" pageSubtitle="Clock in/out, PTO management, and team calendar">
